...
MinimumTotalCost::MinimumTotalCost(std::size_t planning_horizon, std::size_t security_stock, double emision_cost, double hold_cost)
  : BaseMRP(planning_horizon, security_stock, emision_cost, hold_cost)
{}


void MinimumTotalCost::calculate()
{
    // MAKE BATCH TO BATCH
    ...
    while(head<planning_horizon_ && it<planning_horizon_)
    {
        ...
        const double desv_tot {std::abs(cp_tot - ce_tot)};
